...
export async function GET(request: NextRequest) {
  ...
  try {
    const overview = await getWorkspaceOverview({ workspaceId: context.workspace.id, since, until });
    return NextResponse.json({ ok: true, overview });
  } catch (error) {
    return respondWithApiError(await internalError(error, { route: "analytics:overview" }));
  }
}
