...
import { computeGapsForRole, computeTopGapsForRole } from "@/services/skillGapService";
...
export async function GET(request: NextRequest, { params }: { params: { roleId: string } }) {
  ...
  try {
    ...
    const data = await computeGapsForRole({ workspaceId: context.workspace.id, roleId });
    ...
    return respondWithApiError(await internalError(error, { route: "skills-gap:role" }));
  }
}
