...
import { buildCompanyGoalsExport } from "@/services/exportService";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = await buildCompanyGoalsExport({
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=quadrant-goals.csv",
      },
    });
  } catch (error) {
    return respondWithApiError(await internalError(error, { route: "exports:goals" }));
  }
}
