...
import { buildOneOnOnesExport } from "@/services/exportService";
import { toCsv } from "@/lib/csv";
import { requireRole } from "@/services/rbac";

const querySchema = z.object({ periodStart: z.string().optional(), periodEnd: z.string().optional(), managerId: z.string().optional() });
...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = await buildOneOnOnesExport({
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=quadrant-1on1.csv",
      },
    });
  } catch (error) {
    return respondWithApiError(await internalError(error, { route: "exports:1on1" }));
  }
}
