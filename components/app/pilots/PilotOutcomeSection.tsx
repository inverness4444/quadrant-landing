...
export default function PilotOutcomeSection({ pilotId, canEdit }: { pilotId: string; canEdit: boolean }) {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`/api/app/outcomes/pilots/${pilotId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...draft, metrics }),
      });
      const json = await resp.json().catch(() => null);
      if (!resp.ok || !json?.ok) throw new Error(json?.error?.message ?? "Не удалось сохранить");
      setEditing(false);
      void load();
    } catch (err) {
  ...
  const suggest = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`/api/app/outcomes/pilots/${pilotId}/suggest`, { cache: "no-store" });
      const json = await resp.json().catch(() => null);
